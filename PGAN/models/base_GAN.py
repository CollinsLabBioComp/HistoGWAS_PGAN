...
class BaseGAN():
    ...
    def test(self, input, emb, getAvG=False, toCPU=True):
        r"""
        ...
        if getAvG:
            if toCPU:
                return self.avgG(input, emb).cpu()
            else:
                return self.avgG(input, emb)
        elif toCPU:
            return self.netG(input, emb).detach().cpu()
        else:
            return self.netG(input, emb).detach()
    ...
    def optimizeParameters(self, input_batch, inputEmbs, inputLabels=None):
        r"""
        ...
        # #1 Real data
        predRealD = self.netD(self.real_input, self.emb_input)
        ...
        predFakeG = self.netG(inputLatent, self.emb_input).detach()
        ...
        predFakeG = self.netG(inputNoise, self.emb_input)
